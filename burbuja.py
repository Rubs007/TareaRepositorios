...
def calcular_promedio(alumnos):
    total_calificaciones = sum(
        calificacion for nombre, calificacion in alumnos)
    promedio = total_calificaciones / len(alumnos)
    return promedio


def alumnos_sin_derecho(alumnos):
    sin_derecho = [nombre for nombre,
                   calificacion in alumnos if calificacion <= 6]
    return sin_derecho
...
# Archivo de texto
archivo = 'C:/Users/elsai/Documents/codigos/python/Sistemas Dinámicos/Tarea-Repositorios/alumnos.txt'
...
# Mostrar los alumnos sin derecho a exentar el examen
sin_derecho = alumnos_sin_derecho(alumnos)
print("Alumnos sin derecho a exentar el examen:", sin_derecho)
